...
func sendMembers(s *session) error {
	...
	if membersMap, ok := nroom.GetRoom(s.ApiKey, s.Room); ok {
...
func sendSdp(s *session, raw []byte) error {
	...
	dstConn, ok := nroom.GetSession(s.ApiKey, s.Room, message.Args.Dst)
	if !ok {
...
func announceExit(apiKey, room, sessionKey string) {
	...
	if membersMap, ok := nroom.GetRoom(apiKey, room); ok {
		membersMap.Range(func(key, value interface{}) bool {
...
func handleClientMessage(session *session, raw []byte) error {
	...
	default:
		...
		OnClientMessage(session.ApiKey, session.Room, session.SessionKey, raw)
		return nil
	}
}
...
func HandleRootWSUpgrade(w http.ResponseWriter, r *http.Request) {
	...
	defer func() {
		if err := nroom.DestroySession(apiKey, room, sessionKey); err != nil {
			log.Error().Err(err).
				Str("ApiKeyID", apiKey).
				Str("Room", room).
	...
	nroom.StoreSession(apiKey, room, sessionKey, session)
